...
results[["mean_data_1"]][["fastcpd"]] <-
  fastcpd::detect_mean(mean_data_1)@cp_set
```
...
results[["mv_data_1"]][["fastcpd"]] <-
  fastcpd::detect_meanvariance(mv_data_1)@cp_set
```
...
results[["mean_data_3"]][["fastcpd"]] <-
  fastcpd::detect_mean(mean_data_3)@cp_set
```
...
results[["mv_data_3"]][["fastcpd"]] <-
  fastcpd::detect_meanvariance(mv_data_3)@cp_set
```
...
results[["lm_data"]][["fastcpd"]] <-
  fastcpd::detect_lm(lm_data)@cp_set
```
...
results[["binomial_data"]][["fastcpd"]] <-
  fastcpd::detect_binomial(binomial_data)@cp_set
```
...
results[["poisson_data"]][["fastcpd"]] <-
  fastcpd::detect_poisson(poisson_data)@cp_set
```
...
results[["lasso_data"]][["fastcpd"]] <-
  fastcpd::detect_lasso(lasso_data)@cp_set
```
...
results[["ar_data"]][["fastcpd"]] <-
  fastcpd::detect_ar(ar_data, 3)@cp_set
```
...
results[["garch_data"]][["fastcpd"]] <-
  fastcpd::detect_garch(garch_data, c(1, 1))@cp_set
```
...
results[["var_data"]][["fastcpd"]] <-
  fastcpd::detect_var(var_data, 2)@cp_set
```
...
results[["well_log"]] <- list(
  fastcpd = fastcpd::detect_mean(well_log, trim = 0.003)@cp_set,
...
results[["microbenchmark"]] <- microbenchmark::microbenchmark(
  fastcpd = fastcpd::detect_mean(well_log, cp_only = TRUE),
