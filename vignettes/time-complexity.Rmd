...
description: |
  This vignette explores the computational complexity of `detect_mean()`
...
# Time Complexity Simulation for `detect_mean()`

In this section, we generate multivariate normal data with varying lengths and measure the execution time of the `detect_mean()` function. We then create a log-log plot of the execution times and perform a linear regression on the log-transformed data to estimate the power law coefficient.
...
for (i in seq_along(ns)) {
  execution_times[i] <- system.time(detect_mean(
...
ggplot(time_data, aes(x = n, y = time)) +
  ...
  scale_y_log10() +
  labs(
    title = "Time Complexity of detect_mean()",
...
# Impact of SeDG in `detect_lasso()`
...
# Detect change points using detect_lasso() without SeDG (vanilla_percentage = 0)
system.time(result_seg_non_vanilla <- detect_lasso(lasso_data, vanilla_percentage = 0))
#>    user  system elapsed 
#>   5.556   0.794   6.364
...
# Detect change points using detect_lasso() with the vanilla approach (vanilla_percentage = 1)
system.time(result_seg_vanilla <- detect_lasso(lasso_data, vanilla_percentage = 1))
#>    user  system elapsed 
